...
async def test_generate_response_schema_workaround(qwen_client, mock_openai_client):
    ...
    correct_response_content = json.dumps({
        "name": "Bob",
        "age": 25
    })
    
    mock_response_schema = Mock()
    mock_response_schema.choices = [
        Mock(message=Mock(content=schema_response_content))
    ]
    
    mock_response_correct = Mock()
    mock_response_correct.choices = [
        Mock(message=Mock(content=correct_response_content))
    ]
